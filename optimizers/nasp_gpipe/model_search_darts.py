...
class Cell(nn.Module):
    ...
    def __init__(self, steps, C_prev, C, layer, search_space, arch_parameters, output_weights):
    ...
    def forward(self, s0):
        for arch_parameter in self._arch_parameters:
            arch_parameter.data = arch_parameter.to(s0.device, non_blocking=True)

        if update_type.value == update_type.ALPHAS:
            # Normalize mixed_op weights for the choice blocks in the graph
            weights = self._preprocess_op(self._arch_parameters[0])

            # Normalize the output weights
            output_weights = self._preprocess_op(self._arch_parameters[1]) if self._output_weights else None
            # Normalize the input weights for the nodes in the cell
            input_weights = [self._preprocess_op(alpha) for alpha in self._arch_parameters[2:]]
        elif update_type.value == update_type.WEIGHTS:
            # keep weights
    ...
    def _preprocess_op(self, x, discrete=False, normalize=False):
    ...
    def to(self, *args, **kwargs):
        super(Cell, self).to(*args, **kwargs)
        for arch_parameter in self._arch_parameters:
            arch_parameter.data = arch_parameter.to(*args, **kwargs)
...
class GlobalAvgPool(nn.Module):
    def forward(self, input):
        return input.view(*input.shape[:2], -1).mean(-1)
    ...
    def __init__(self, C, num_classes, layers, criterion, output_weights, search_space, steps=4):
        ...
        for i in range(layers):
            if i in [layers // 3, 2 * layers // 3]:
                ...
                reduce = nn.MaxPool2d(kernel_size=2, stride=2, padding=1)
                self.cells.add_module(name, reduce)
            name = 'normal_{}'.format(i)
            arch_parameters = self._arch_parameters[i*len_arch:(i+1)*len_arch]
            cell = Cell(steps=self._steps, C_prev=C_prev, C=C_curr, layer=i, search_space=search_space,
                        arch_parameters=arch_parameters, output_weights=output_weights)
        ...
        self.global_pooling = GlobalAvgPool()
